...
export default function ParticipantList({ participants, revealVotes, timer, allVoted }: IParticipantListModel) {
  ...
  const remainingCount = useMemo(() => participants.filter((p) => !p.hasVoted).length, [participants]);
  const showWaitingHint = useMemo(() => notEveryoneVoted && hasTimer, [notEveryoneVoted, hasTimer]);

  return (
    <motion.div className="w-full max-w-2xl space-y-2" variants={listVariants} initial="hidden" animate="visible">
