        ...
        {isLoadingRoom ? (
          ...
          <div className="flex flex-col lg:flex-row gap-6 w-full items-center justify-between">
            <div className="w-full lg:w-1/3">
              <ParticipantList participants={participants} revealVotes={allVoted} timer={timer} allVoted={allVoted} />
            </div>

            <div className="flex-1 flex flex-col items-center gap-4">
              <div className="flex gap-3 flex-wrap justify-center">
                {CARD_VALUES.map((value) => (
                  <Card key={value} value={value} onClick={() => sendVote(value)} disabled={allVoted || timer === 0} />
                ))}
              </div>

              {timer === null ? <Loader label="Connecting to room..." /> : <TimerDisplay seconds={timer} />}
